...
function makeWatermarkSvg(w, h) {
  const fs   = Math.max(18, Math.floor(w / 10))
  const cx   = Math.round(w / 2)
  const cy   = Math.round(h / 2)
  const cy2  = Math.round(h * 0.62)
  const fs2  = Math.round(fs * 0.5)
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
  <text x="${cx}" y="${cy}"
    font-family="Arial,sans-serif" font-size="${fs}px" font-weight="bold"
    fill="white" fill-opacity="0.38" text-anchor="middle" dominant-baseline="middle"
    transform="rotate(-28,${cx},${cy})">STUDIO PREVIEW</text>
  <text x="${cx}" y="${cy2}"
    font-family="Arial,sans-serif" font-size="${fs2}px"
    fill="white" fill-opacity="0.25" text-anchor="middle" dominant-baseline="middle"
    transform="rotate(-28,${cx},${cy2})">vayustudio.com</text>
</svg>`)
}
