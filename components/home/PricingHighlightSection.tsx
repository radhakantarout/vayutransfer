...
const POINTS = [
  <>New accounts start with <b className="text-text-primary font-semibold">₹50 free credit</b>, no card required.</>,
...
export default function PricingHighlightSection({ balancePaise }: Props) {
  return (
      ...
      <div className="grid md:grid-cols-2 rounded-3xl overflow-hidden border border-border bg-card">
        <div className="p-9 md:p-11 border-b md:border-b-0 md:border-r border-border">
          <div className="flex items-baseline gap-2.5 font-mono font-semibold text-4xl md:text-5xl tracking-tight text-text-primary">
            ₹4.99
            <span className="text-base font-sans font-normal text-muted">/ GB, exact size, no tiers</span>
          </div>
          <h3 className="font-display text-lg font-semibold text-text-primary mt-6 mb-3">One rate. No plans to pick.</h3>
          <p className="text-sm text-muted leading-relaxed">
            No free tier, no monthly limit, no per-transfer cap — every transfer costs exactly its size × ₹4.99/GB, calculated to the megabyte, deducted from your wallet.
          </p>
        ...
        <div className="p-9 md:p-11 flex flex-col justify-center">
          <div className="rounded-2xl border border-border bg-bg p-6 mb-6">
            ...
            <div className="text-xs text-muted mt-3">
              {typeof balancePaise === 'number'
                ? 'Deducted only as you upload, calculated to the exact GB'
