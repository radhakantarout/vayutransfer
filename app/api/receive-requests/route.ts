...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = await req.json().catch(() => ({})) as { message?: string; expiryDays?: number }
    ...
    void logAudit({
      ...
      outcome: 'success',
      walletId: user.walletId,
      metadata: { requestId, expiryDays },
    })

    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000'
    return NextResponse.json<ApiResponse<{ requestId: string; receiveLink: string; expiryTime: string }>>({
      success: true,
      data: { requestId, receiveLink: `${appUrl}/receive/${requestId}`, expiryTime: receiveRequest.expiryTime },
    })
  } catch (err) {
