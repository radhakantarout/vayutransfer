...
export default function Zari({ site }: { site: StudioWebsite }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: bg.bg, color: fontColor, fontFamily: `"Cormorant Garamond", Georgia, serif, ${MULTI_SCRIPT_FONT_FALLBACK}` }}>
      ...
      {/* Nav */}
      <header data-preview-tab="content" className="fixed top-0 left-0 right-0 z-50 border-b" style={{ background: `${bg.bg}f2`, borderColor: `${accent}33` }}>
      ...
      {/* Hero */}
      <section data-preview-tab="content" className="pt-32 pb-20 px-6" style={{ background: sx('hero')?.background }}>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg opacity-70 max-w-lg mx-auto mb-2" style={emphasisTextStyle(sx('hero')?.emphasis)}>{site.heroSubtitle}</p>
          {site.tagline && <p className="text-sm italic opacity-50 mb-10" style={emphasisTextStyle(sx('hero')?.emphasis)}>{site.tagline}</p>}
          {site.bookingEnabled && (
      ...
      <section id="about" data-preview-tab="content" className="py-24 px-6" style={{ background: sx('about')?.background ?? bg.alt }}>
        <Reveal className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-2xl font-light leading-relaxed" style={emphasisTextStyle(sx('about')?.emphasis)}>{site.about}</p>
      ...
      {site.services.length > 0 && (
        <section id="services" data-preview-tab="services" className="py-24 px-6" style={{ background: sx('services')?.background }}>
          <Reveal className="max-w-3xl mx-auto">
            <p className="text-xs uppercase tracking-[0.35em] text-center mb-12" style={{ color: accent, ...emphasisTextStyle(sx('services')?.emphasis) }}>{t('sectionServicesHeading', 'Services')}</p>
      ...
      {!!site.testimonials?.length && (
        <section id="reviews" data-preview-tab="testimonials" className="py-24 px-6" style={{ background: sx('testimonials')?.background ?? bg.alt }}>
          <Reveal className="max-w-5xl mx-auto">
            <p className="text-xs uppercase tracking-[0.35em] text-center mb-12" style={{ color: accent, ...emphasisTextStyle(sx('testimonials')?.emphasis) }}>{t('sectionReviewsHeading', 'What Our Clients Say')}</p>
      ...
      <section id="book" data-preview-tab={site.bookingEnabled ? 'booking' : 'contact'} className="py-24 px-6" style={{ background: sx('book')?.background }}>
        <Reveal className="max-w-xl mx-auto text-center">
          <p className="text-xs uppercase tracking-[0.35em] mb-4" style={{ color: accent, ...emphasisTextStyle(sx('book')?.emphasis) }}>
            {site.bookingEnabled ? t('sectionBookHeadingEnabled', 'Reserve a Consultation') : t('sectionBookHeadingDisabled', 'Get in Touch')}
          </p>
          <h2 className="text-3xl font-light mb-10" style={emphasisTextStyle(sx('book')?.emphasis)}>{t('bookingIntro', "We'd love to hear your story")}</h2>
          {site.bookingEnabled
