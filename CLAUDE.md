...
Last updated: 2026-09-20 (STEP 16 close-out)
...
[x] STEP 16 — VayuStudios Moments: prompt-first "Reel It" redesign + text-to-video (5 phases), real Kling reel-generation refund/retry-cascade bug fix, and AI Image Studio provider switch Kling→OpenAI gpt-image-2.5-sunburst — MERGED TO MAIN 2026-09-20 (merge commit d755f097), live in production. Both `vayustudio-reelgen` and `vayustudio-imagegen` production Lambdas explicitly redeployed the same day (code merges never auto-deploy Lambdas). `OPENAI_API_KEY` still needs adding to Vercel Production env vars — see priorities below.
```
...
- VayuTransfer pricing model, simplified further on 2026-08-12 (removing the earlier 10GB/month + 3GB/transfer free-quota rewrite entirely): every transfer costs exactly `billableGB × ₹4.99/GB`, calculated precisely from the real byte size (e.g. 100MB → ₹0.50) — no free tier, no monthly quota, no per-transfer cap, no `isFree` branch anywhere. `calculatePrice(fileSizeBytes)` now takes just the one argument. New signups' ₹50 bonus is plain wallet balance, nothing tracked separately. Removed entirely: `Wallet.freeQuotaUsedBytes`/`freeQuotaMonthKey`, `PriceBreakdown.isFree`/`freeQuotaRemainingBytes`, `Transfer.isFreeTransfer`, `lib/wallet.ts#getFreeQuotaUsedBytes`/`consumeFreeQuota`, `FREE_QUOTA_MONTHLY_BYTES`/`FREE_TRANSFER_MAX_BYTES`. Downloads remain unpriced/uncapped by a user-visible "slot count" — unlimited until the link expires, backed only by a silent `MAX_DOWNLOADS_PER_LINK=200` anti-abuse ceiling never shown in the UI.
...
- **Pricing simplification (2026-08-12)**: removed the old 10GB/month free-quota + 3GB/transfer free-cap model entirely. Every transfer now costs exactly `roundUpToTenthGB(bytes) × ₹4.99/GB` with no free tier — the ₹50 new-signup bonus is just ordinary wallet balance. Removed: `Wallet.freeQuotaUsedBytes`/`freeQuotaMonthKey`, `PriceBreakdown.isFree`/`freeQuotaRemainingBytes`, `Transfer.isFreeTransfer`, `lib/wallet.ts#getFreeQuotaUsedBytes`/`consumeFreeQuota`, `FREE_QUOTA_MONTHLY_BYTES`/`FREE_TRANSFER_MAX_BYTES`. `calculatePrice(fileSizeBytes)` is now single-argument. `MAX_DOWNLOADS_PER_LINK=200` is unchanged/unrelated. Every marketing page (home, /pricing, /about, /support) rewritten to match. Built and typechecked on `develop`, not yet pushed.
- **Download-All fix (2026-08-12, built not yet deployed)**: root cause was a staggered `setTimeout`+`<a download>` loop with no per-click user gesture — Chrome silently throttles/blocks multi-file automatic downloads past a handful. New size-based strategy in `DownloadCard.tsx` (mirrors VayuStudios' proven print-portal pattern, kept as fully separate code): batches under 1GB zip entirely client-side via `JSZip` (already a dependency) using `relativePath` as the in-zip path so folder hierarchy is preserved; batches at/above 1GB hand off to a new dedicated `vayu-transfer-zip` Lambda (`lambda/vayu-transfer-zip/`, own code/table, zero shared runtime with VayuStudios' `vayustudio-zip`) via `POST /api/download/[fileId]/zip` → poll `GET /api/download/[fileId]/zip/status/[jobId]` every 2s. New `ZipJob` type + `vayu-zip-jobs` DynamoDB table (env `DYNAMO_ZIP_JOBS_TABLE`), new `Transfer.zipJobId` field (caches the built zip so repeat "Download All" clicks reuse it instead of rebuilding). Single-file downloads and per-row individual downloads inside a batch are unchanged. **Provisioning status (2026-08-12)**: `vayu-zip-jobs` and `vayu-zip-jobs-test` DynamoDB tables are created and ACTIVE (same PAY_PER_REQUEST/no-GSI/no-TTL shape as `vayustudio-jobs`) — confirmed zero impact to any `vayustudio-*` table (table count check before/after). **Still not live** — needs: (1) create the `vayu-transfer-zip` AND `vayu-transfer-zip-test` Lambda functions in the AWS console (VayuStudios' equivalent — `vayustudio-zip`/`vayustudio-zip-test` — is two separate functions per environment, not one shared function, so this needs the same; `deploy.sh` only supports `update-function-code`, so initial creation is manual per function), then run `FUNCTION_NAME=vayu-transfer-zip-test DYNAMO_ZIP_JOBS_TABLE=vayu-zip-jobs-test lambda/vayu-transfer-zip/deploy.sh` (and again without the overrides for production) to push code + config (Node.js 20.x, 900s timeout, 3008MB memory) to each, (2) attach an IAM role to each with DynamoDB access to its own zip-jobs table + basic Lambda execution (no S3/R2 IAM needed — R2 creds arrive per-invoke in the payload), (3) set `TRANSFER_ZIP_LAMBDA_ARN` (the function's own ARN) and `DYNAMO_ZIP_JOBS_TABLE` in Vercel per environment: Production → `vayu-transfer-zip`'s ARN + `vayu-zip-jobs`; Preview (test.vayutransfer.com) → `vayu-transfer-zip-test`'s ARN + `vayu-zip-jobs-test`. Lambda functions themselves not yet created — code is on `develop`, unpushed.

### Next Session Priorities
0. **Add `OPENAI_API_KEY` to Vercel Production env vars** — STEP 16 (merged to main 2026-09-20) switched AI Image Studio's primary provider to OpenAI, and without this key set in Production, every "Edit with AI"/generate call will fail there (Preview/test.vayutransfer.com needs it too, for the same reason — see STEP 16 for context). Both production Lambdas (`vayustudio-imagegen`, `vayustudio-reelgen`) were already explicitly redeployed same day.
0a. Provision AWS for the new Download-All zip fix (see "Download-All fix" decision above) — `vayu-zip-jobs` DynamoDB table, `vayu-transfer-zip` Lambda (create in console, then run its `deploy.sh`), IAM role, and `TRANSFER_ZIP_LAMBDA_ARN`/`DYNAMO_ZIP_JOBS_TABLE` Vercel env vars. Code is done and typechecked on `develop` but unpushed and untested against real infra — needs the user's explicit go-ahead on each resource per the standing AWS-confirmation rule.
